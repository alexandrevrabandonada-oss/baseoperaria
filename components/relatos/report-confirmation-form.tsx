...
import { confirmReportAction } from "@/app/relatos/actions";
import { Button } from "@/components/ui/button";
import { reportConfirmationOptions, type ReportConfirmationType } from "@/types/relatos";

import type { ReportConfirmationActionState } from "@/types/relatos";
...
type ReportConfirmationFormProps = {
  isCreator: boolean;
  myConfirmation: ReportConfirmationType | null;
  reportId: string;
};

export function ReportConfirmationForm({
  isCreator,
  myConfirmation,
  reportId,
}: ReportConfirmationFormProps) {
  const [state, formAction] = useActionState<
    ReportConfirmationActionState,
    FormData
  >(confirmReportAction, {});
  ...
  if (isCreator) {
    return (
      <div className="rounded-2xl border bg-muted p-4 text-sm text-muted-foreground">
        O autor do relato não faz confirmação própria. Outros usuários autenticados podem confirmar este item.
      </div>
    );
  }
